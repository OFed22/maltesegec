...
# Extended mode specific parameter
...
# Authentic data paths - Qari tal-Provi and Busuttil
INPUT_SENTENCE_FILE="/app/malteseGEC/data/authentic/splits/train/qari_busuttil_src.txt"
TARGET_SENTENCE_FILE="/app/malteseGEC/data/authentic/splits/train/qari_busuttil_trg.txt"
...
NO_EDIT_DATA_DIR=${DATA_DIR}/$PROBLEM-$LANG-$MODE-finetune-tep$TOKEN_ERR_PROB-ted$TOKEN_ERR_DISTRIBUTION-cep$CHAR_ERROR_PROB-ced$CHAR_ERR_DISTRIBUTION-aas${ADDITIONAL_ARTIFICIAL_SENTENCES}-dr${DATA_RATIO}
DATA_DIR_WITH_WEIGHT=${NO_EDIT_DATA_DIR}-${EDIT_WEIGHT}

# Pretrained model directory (where your extended model checkpoints are)
...
TRAIN_DIR_FINETUNED=${TRAIN_DIR}/$PROBLEM-$LANG-$MODE-finetune-tep$TOKEN_ERR_PROB-ted$TOKEN_ERR_DISTRIBUTION-cep$CHAR_ERROR_PROB-ced$CHAR_ERR_DISTRIBUTION/$MODEL-$MODEL_TYPE-iwdr${INPUT_WORD_DROPOUT_RATE}-twdr${TARGET_WORD_DROPOUT_RATE}-ew${EDIT_WEIGHT}-ws${WARMUP_STEPS}-lrc${LEARNING_RATE_CONSTANT}-aas${ADDITIONAL_ARTIFICIAL_SENTENCES}-dr${DATA_RATIO}
