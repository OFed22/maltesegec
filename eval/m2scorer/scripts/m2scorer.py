...
for line in fin:
    ...
    if line:  # Skip empty lines
        system_sentences.append(line)
fin.close()

print(f"System sentences: {len(system_sentences)}")
print(f"Source sentences: {len(source_sentences)}")
print(f"Gold edits: {len(gold_edits)}")
if len(system_sentences) != len(source_sentences):
    print(f"Mismatch! First system: '{system_sentences[0] if system_sentences else 'EMPTY'}'")
    print(f"Last system: '{system_sentences[-1] if system_sentences else 'EMPTY'}'")
